            ...
            <td colspan="2">
                <table class="inner-table">
                    ...
                    <tbody>
                        ...
                        @foreach ($CXC->budget as $index => $budgets)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td style="text-align:left;">{{ $budgets->patient->first_name }}
                                    {{ $budgets->patient->last_name }}</td>
                                <td>{{ $budgets->type }}</td>
                                <td style="text-align:left;">{{ $budgets->doctor->name }}
                                    {{ $budgets->doctor->last_name }}</td>
                                <td style="text-align:left;">{{ $budgets->emission_date }}</td>
                                <td style="text-align:left;">$ {{ number_format($budgets->total, 2) }}</td>
                            </tr>

                            @php $sumaTotal += $budgets->total; @endphp

                            @if (($index + 1) % 10 === 0 && $index + 1 !== count($CXC->budget))
                    </tbody>
                </table>
            </td>
        </tr>
